...
move $s0, $zero# i = 0
for1tst:slt $t0, $s0,$s3 #reg$t0=0if$s0ä$s3(iän)
beq $t0, $zero, exit1# go to exit1 if $s0 ä $s3 (i ä n)
addi $s1, $s0,-1# j = i ñ 1
...
slt $t0, $t4, $t3 # reg $t0 = 0 if $t4 ä $t3
beq $t0, $zero, exit2# go to exit2 if $t4 ä $t3
